...
def process_task(task, sub_kind_items, sources, info=None):
    ...
    try:
        task["sub_kind_item"] = sub_kind_item_mapping(
            task["name"], sub_kind_items, info
        )
...
def estimate_data_predict(tasks, sub_kind_items, sources, info=None):
    ...
    with concurrent.futures.ThreadPoolExecutor(
        ...
        futures = [
            executor.submit(process_task, task, sub_kind_items, sources, info)
            for task in tasks
        ]
        results = [
